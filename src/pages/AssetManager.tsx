import { Download, Edit, FileText, Film, Filter, Grid, Image, List, Music, Plus, Search, Trash, Upload, X } from 'lucide-react';
...
const AssetManager = () => {
  ...
  const mockAssets = [
    { id: 1, type: 'image', title: 'Company Logo', tags: ['logo', 'brand'], url: '/api/placeholder/150/150', dateAdded: '2025-04-15' },
    { id: 2, type: 'image', title: 'Team Photo', tags: ['team', 'people'], url: '/api/placeholder/150/150', dateAdded: '2025-04-10' },
    { id: 3, type: 'video', title: 'Product Demo', tags: ['product', 'demo'], url: '/api/placeholder/150/150', dateAdded: '2025-04-05' },
    { id: 4, type: 'audio', title: 'Background Music', tags: ['music', 'background'], url: '/api/placeholder/150/150', dateAdded: '2025-04-01' },
    { id: 5, type: 'lottie', title: 'Loading Animation', tags: ['animation', 'loading'], url: '/api/placeholder/150/150', dateAdded: '2025-03-28' },
    { id: 6, type: 'image', title: 'Product Image 1', tags: ['product', 'gallery'], url: '/api/placeholder/150/150', dateAdded: '2025-03-25' },
    { id: 7, type: 'image', title: 'Product Image 2', tags: ['product', 'gallery'], url: '/api/placeholder/150/150', dateAdded: '2025-03-25' },
    { id: 8, type: 'video', title: 'Testimonial Video', tags: ['testimonial', 'client'], url: '/api/placeholder/150/150', dateAdded: '2025-03-20' },
  ];
  ...
  const getAssetIcon = (type) => {
    switch (type) {
      ...
      case 'lottie': return <FileText size={24} className="text-orange-500" />;
            ...
            <div className="relative">
              ...
              {filterOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                  ...
                  <div className="p-3">
                    {assetTypes.map(type => (
                      <div key={type.id} className="flex items-center mb-2 last:mb-0">
                        <input
                          type="checkbox"
                          id={`type-${type.id}`}
                          className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                        />
                        <label htmlFor={`type-${type.id}`} className="ml-2 flex items-center text-gray-700">
                          {type.icon}
                  ...
                  <div className="p-3 border-t border-gray-200 flex justify-end">
                    <button className="px-3 py-1 text-sm bg-blue-600 text-white rounded-md">Apply Filters</button>
                  </div>
                </div>
              )}
            </div>
        ...
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {mockAssets.map(asset => (
              <div 
                key={asset.id} 
                className={`bg-white rounded-lg border ${selectedAsset === asset.id ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200'} overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer`}
                ...
                <div className="relative h-36 bg-gray-100 flex items-center justify-center">
                  ...
                  {asset.type !== 'image' && (
                    <div className="flex flex-col items-center justify-center p-4">
                      ...
                      <span className="mt-2 text-xs text-gray-500 capitalize">{asset.type}</span>
                    </div>
                  )}
                </div>
                <div className="p-3">
                  <h3 className="font-medium text-gray-800 truncate">{asset.title}</h3>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {mockAssets.map(asset => (
                  <tr 
                    key={asset.id} 
                    className={`hover:bg-gray-50 ${selectedAsset === asset.id ? 'bg-blue-50' : ''}`}
                    onClick={() => setSelectedAsset(asset.id)}
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-sm text-gray-700 capitalize">{asset.type}</span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-lg">
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-800">Upload New Asset</h2>
              <button 
            ...
            <div className="p-6">
              <div className="mb-6">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {assetTypes.map(type => (
                    ...
                    >
                      {type.icon}
                      <span className="mt-1 text-xs text-gray-700">{type.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <p className="mt-1 text-xs text-gray-500">Tags help organize and find assets later</p>
              </div>
            </div>
        ...
        <div className="fixed inset-y-0 right-0 w-80 bg-white border-l border-gray-200 overflow-auto z-20">
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="aspect-w-16 aspect-h-9 bg-gray-100 rounded-md flex items-center justify-center overflow-hidden">
              ...
              <img src="/api/placeholder/300/200" alt="Selected asset preview" className="object-contain w-full h-full" />
            </div>
          </div>
